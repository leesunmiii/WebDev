...
public class GoodsAllServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(GoodsVO vo:list)
		{
			...
			out.write("<a href=MainServlet?mode=5&no="+vo.getNo()+"&type=1>"); // 클릭할 때 상세보기 넘어가는 (링크 걸리는 위치)
			//							   ------ ------------------------- 
			//							메인이 받아야하는	디테일이 받아야하는
		...
		{	
			out.write("<li><a href=MainListServlet?page="+(startpage-1)+"&mode=1>&lt;</a></li>");
			// 11page => 이전 => 10
		}
		...
		{
			out.write("<li><a href=MainListServlet?page="+(endpage+1)+"&mode=1>&gt;</a></li>");
		}
